...
{!user && (
  ...
  sx={{
    ...
    '& .MuiTabs-indicator': { backgroundColor: '#FF4151' },
    '& .MuiTab-root': {
...
>
        ...
        <Box sx={{ display: 'flex', maxHeight: '500px', flexDirection: 'column', gap: '2px', mb: 1.5, overflow: 'scroll' }}>
          {habits.length === 0 ? (
            <Typography sx={{ color: 'rgba(255,255,255,0.3)', fontSize: 13, padding: '12px' }}>No habits yet — add one below</Typography>
          ) : (
            habits.map((habit) => (
              <Box
                key={habit.id}
              ...
              >
                ...
                <Box sx={{
                  ...
                  backgroundColor: habit.streak >= 7 ? 'rgba(255,65,81,0.12)' : habit.streak > 0 ? 'rgba(255,165,0,0.12)' : 'rgba(255,255,255,0.06)',
                  color: habit.streak >= 7 ? '#FF4151' : habit.streak > 0 ? '#FFA500' : 'rgba(255,255,255,0.4)',
                  fontSize: 10, fontWeight: 500, whiteSpace: 'nowrap',
                }}>
                  <Typography sx={{ fontSize: 10, fontWeight: 500, color: 'inherit' }}>
                    {habit.streak ?? 0}d
                  </Typography>
                </Box>
                ...
                <Tooltip title={`Last completed: ${habit.lastChecked || 'never'}`}>
                  <IconButton size="small" onClick={(e) => handleMenuOpen(e, habit.id)}>
                    <MoreVertIcon fontSize="small" />
                  </IconButton>
                </Tooltip>

                <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
                  <MenuItem onClick={() => selectedHabit && removeProgress(selectedHabit, handleMenuClose)}>Undo</MenuItem>
                  <MenuItem onClick={() => selectedHabit && deleteHabit(selectedHabit, handleMenuClose)}>Delete</MenuItem>
                </Menu>
              </Box>
            ))
          )}
        ...
        >
          <Box sx={{ width: 18, height: 18, borderRadius: '4px', backgroundColor: 'rgba(255,65,81,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FF4151', fontSize: 14 }}>+</Box>
          <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.25)' }}>Add a new habit</Typography>
        </Box>
    ...
    {/* Journal tab */}
    {mobileTab === 1 && (
  <Box sx={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
  ...
  /* ── Desktop layout — your existing three column layout ── */
  <Box sx={{ display: 'flex', height: 'calc(100vh - 56px)', overflow: 'hidden' }}>
    {/* heatmap sidebar */}
    <Box sx={{ width: '420px', flexShrink: 0, borderRight: '0.5px solid rgba(255,255,255,0.06)', padding: '24px', overflowY: 'auto' }}>
    ...
    <Box sx={{ flex: 1, overflowY: 'auto', padding: '24px 48px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <Box sx={{ width: '100%', maxWidth: '520px' }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: '2px', mb: 2 }}>
          {habits.length === 0 ? (
            <Typography sx={{ color: 'rgba(255,255,255,0.3)', fontSize: 14, padding: '16px' }}>No habits yet — add one below</Typography>
          ) : (
            habits.map((habit) => (
              <Box
                key={habit.id}
        ...
        >
          <Box sx={{ width: 22, height: 22, borderRadius: '6px', backgroundColor: 'rgba(255,65,81,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FF4151', fontSize: 18 }}>+</Box>
          <Typography sx={{ fontSize: 14, color: 'rgba(255,255,255,0.25)' }}>Add a new habit</Typography>
        </Box>
      </Box>
    </Box>
    ...
    {/* journal sidebar */}
   <Box sx={{ width: '100%', maxWidth: '450px',display: 'flex',  flexShrink: 0, borderLeft: '0.5px solid rgba(255,255,255,0.06)', padding: '24px', overflowY: 'auto' }}>
  <JournalPanel
    journalEntries={journalEntries}
    journalText={journalText}
    setJournalText={setJournalText}
